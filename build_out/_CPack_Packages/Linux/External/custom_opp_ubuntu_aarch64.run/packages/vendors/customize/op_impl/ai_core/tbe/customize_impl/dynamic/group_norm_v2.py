...
def _build_args(x_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, num_groups, data_format, eps, is_training):
    __inputs__ = []
...
@tbe_register.register_operator("GroupNormV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def group_norm_v2(x_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, num_groups, data_format="NCHW", eps=0.0001, is_training=True, kernel_name="group_norm_v2", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, num_groups, data_format, eps, is_training)
    ...
    origin_func_name = "group_norm_v2"
    ascendc_src_dir_ex = "group_norm_v2"
    ascendc_src_dir = "group_norm_v2"
    ascendc_src_file = "group_norm_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator GroupNormV2. kernel name is " + kernel_name)
    op_type = "GroupNormV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, gamma_in__, beta_in__], origin_outputs = [y_out_, mean_out_, rstd_out_], param_type_dynamic = False,\
        mc2_ctx = None, param_type_list = ['required', 'required', 'required', 'required', 'required', 'required'])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, num_groups, data_format="NCHW", eps=0.0001, is_training=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, num_groups, data_format, eps, is_training)
    result = check_op_cap("op_select_format", "GroupNormV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, num_groups, data_format="NCHW", eps=0.0001, is_training=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, num_groups, data_format, eps, is_training)
    result = check_op_cap("get_op_specific_info", "GroupNormV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
